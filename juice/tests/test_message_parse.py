...
class TestMixer(TestCase):
  ...
  def test_muting_set(self):
    ...
    parse_msg('00%3A04%3A20%3A23%3A30%3A7f mixer muting 0'))

  def test_muting_toggle(self):
    self.assertEqual({
      'player': {
        'id': '00:04:20:23:30:7f',
        'muting': 'toggle',
    ...
    parse_msg('00%3A04%3A20%3A23%3A30%3A7f mixer muting toggle'))
...
class TestGenres(TestCase):
  ...
  def test_artist(self):
    self.assertEqual({
      ...
      parse_msg('genres 0 6 artist_id%3A5 id%3A2 genre%3AFilm id%3A1 genre%3AJazz count%3A2'))
